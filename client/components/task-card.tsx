...
export function TaskCard({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`relative group rounded-xl p-4 transition-all border shadow-sm hover:shadow-md ${
        task.completed
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2">
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="w-7 h-7 rounded hover:bg-yellow-100 dark:hover:bg-yellow-900/30 flex items-center justify-center text-sm"
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                className="w-7 h-7 rounded hover:bg-purple-100 dark:hover:bg-purple-900/30 flex items-center justify-center text-sm"
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                className="w-7 h-7 rounded hover:bg-blue-100 dark:hover:bg-blue-900/30 flex items-center justify-center text-sm"
                title="Edit"
              >
                ✏️
              </motion.button>
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                className="w-7 h-7 rounded hover:bg-gray-100 dark:hover:bg-gray-700/30 flex items-center justify-center text-sm"
                title="Archive"
              >
                📦
              </motion.button>
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                className="w-7 h-7 rounded hover:bg-red-100 dark:hover:bg-red-900/30 flex items-center justify-center text-sm"
                title="Delete"
              >
                🗑️
              </motion.button>
          ...
          <div className="flex flex-wrap gap-2 items-center mb-2">
            {/* Priority Badge */}
            <span className={`text-xs font-medium px-2 py-1 rounded-full ${priorityConfig[task.priority].color}`}>
